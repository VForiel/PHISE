...
def time_evolution(ctx:Context=None, n=100, map=np.median) -> np.ndarray:
    ...
    -------
    np.ndarray
        The time evolution of the kernel nuller.
    """
    ...
    data = np.empty((len(h_range), 3))
    ref_data = np.empty((len(h_range), 3))
    ...
    for k in range(3):
        axs[k].scatter(h_range, data[:, k], label='Data')
        axs[k].plot(h_range, ref_data[:, k], label='Reference', linestyle='--')
