...
def gui(ctx: Context=None):
    ...
    for i in range(3):
        α_slider = widgets.FloatSlider(value=0.0, min=-180.0, max=180.0, step=0.1, description=f'α{i + 1} (deg):', continuous_update=False)
        θ_slider = widgets.FloatSlider(value=2.0, min=0.0, max=10.0, step=0.01, description=f'θ{i + 1} (mas):', continuous_update=False)
...
"""
        ...
        for (i, (α_slider, θ_slider, c_slider)) in enumerate(companion_parameters_sliders[:nb_companions]):
            ...
            c = 10 ** c_slider.value
            tmp_ctx.target.companions.append(Companion(c=c, θ=θ, α=α, name=f'Companion {i + 1}'))
...
"""
    ...
    display(widgets.VBox([widgets.Label('Select the number of companions:'), nb_companion_selector, *[widgets.HBox([α_slider, θ_slider, c_slider]) for (α_slider, θ_slider, c_slider) in companion_parameters_sliders], widgets.HBox([reset_button, status_label]), widgets.Label('Transmission Maps (at h=0):'), transmission_plot, widgets.Label('Temporal Response:'), temporal_response_plot]))
    update_plot()

def fit(ctx: Context, α_guess: u.Quantity=0 * u.rad, θ_guess: u.Quantity=2 * u.mas, c_guess: float=1e-06):
    """"fit.
...
"""
        (α, θ) = params
        ideal_ctx.target.companions = [Companion(c=c_guess, θ=θ * u.mas, α=α * u.deg, name='Companion')]
